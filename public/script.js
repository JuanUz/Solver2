document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NUEVA FUNCIÓN: Embellecedor Matemático ---
    // Limpia la sintaxis de Python para que los humanos la lean mejor
    function formatMath(str) {
        if (!str) return "";
        return str
            .replace(/\*\*/g, '^') // Cambia potencias de Python (**) a un formato normal (^)
            .replace(/\*/g, '');   // Elimina los asteriscos de multiplicación (*)
    }
    ...
    btnSolve.addEventListener('click', async () => {
        ...
        try {
            ...
            if (data.status === "not_exact") {
                alert(`⚠️ La ecuación NO es exacta (∂M/∂y = ${formatMath(data.dM_dy)} ≠ ∂N/∂x = ${formatMath(data.dN_dx)}).\n${data.mensaje}`);
                btnSolve.textContent = "Resolver Ecuación";
                btnSolve.disabled = false;
                return;
            }

            // 4. Renderizado Dinámico aplicando la función formatMath() a cada resultado
            let stepsHtml = '';
            ...
            stepsHtml += `<div class="result-box" style="margin-bottom: 25px;">
                ...
                <ul style="list-style-type: none; padding-left: 0; font-family: monospace; font-size: 0.95rem;">
                    <li style="margin-bottom: 8px; background: #0a0b0d; padding: 10px; border-radius: 6px; border: 1px solid #333;">
                        <span style="color: var(--text-main);">∂M/∂y = </span> <span style="color: var(--primary);">${formatMath(data.dM_dy)}</span>
                    </li>
                    <li style="margin-bottom: 8px; background: #0a0b0d; padding: 10px; border-radius: 6px; border: 1px solid #333;">
                        <span style="color: var(--text-main);">∂N/∂x = </span> <span style="color: var(--primary);">${formatMath(data.dN_dx)}</span>
                    </li>
                </ul>
            ...
            stepsHtml += `<div class="result-box" style="margin-bottom: 25px;">
                ...
                <ul style="list-style-type: none; padding-left: 0; font-family: monospace; font-size: 1rem;">
                    <li style="margin-bottom: 8px; background: #0a0b0d; padding: 12px; border-radius: 6px; border: 1px solid #333;">
                        1. Integrando M: f(x,y) = <strong style="color: white;">${formatMath(data.f_partial)} + g(y)</strong>
                    </li>
                    <li style="margin-bottom: 8px; background: #0a0b0d; padding: 12px; border-radius: 6px; border: 1px solid #333;">
                        2. Igualando y despejando g'(y): g'(y) = <strong style="color: white;">${formatMath(data.g_prime)}</strong>
                    </li>
                    <li style="margin-bottom: 8px; background: #0a0b0d; padding: 12px; border-radius: 6px; border: 1px solid #333;">
                        3. Integrando g'(y): g(y) = <strong style="color: white;">${formatMath(data.g_y)}</strong>
                    </li>
                </ul>
            </div>`;
            ...
            solutionOutput.innerHTML = `<div class="result-box" style="border-left: 4px solid var(--primary);">
                ...
                <div style="background: #0a0b0d; padding: 15px; border-radius: 8px; text-align: center; border: 1px solid #333;">
                    <strong style="color: var(--primary); font-size: 1.5rem;">${formatMath(data.solucion)}</strong>
                </div>
            </div>`;

            resultsSection.classList.remove('hidden');

            const systemPrompt = `Eres "HamsterSolver", tutor de Ciencias Básicas. El usuario resolvió la EDO Exacta con M=${mEq} y N=${nEq}. El resultado calculado es ${formatMath(data.solucion)}. Responde dudas teóricas breves.`;
